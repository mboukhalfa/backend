...
def migrate(container_name, num_iteration, ip_sdn_controller, target_cloud="None"):
    ...
    if not helpers.name_control(container_name):
        ...
        ip_server_container = helpers.matching_ip(container_name)
        server_device, server_port_number, ip_vm_server = onos.sdn_host_information(
            ip_sdn_controller, ip_server_container)
        ...
        ip_client_container = helpers.matching_ip(helpers.matching(container_name))
        client_device, client_port_number, ip_vm_client = onos.sdn_host_information(
            ip_sdn_controller, ip_client_container)
        ...
        if rmq.management_task(ip_source, "migration"):
            ...
            result = rmq.migration(container_name, ip_destination, num_iteration, ip_source)

        if target_cloud == "None":
            target_cloud = dashboard_helper.get_iaas_ip_match(str(ip_destination))
        ...
        intents.network_redirection_1(ip_sdn_controller, client_device, mac_ovs_destination, client_port_number,
                                      server_port_number, vxlan_port, ip_client_container, ip_server_container,
                                      int(intent_priority))
        ...
        if answer != 1:
            ...
            return "Error"
        while helpers.store_db_log(id_request, str(result), str(target_cloud)) != "0":
            print("DB not yet updated")
        if result != 1:
